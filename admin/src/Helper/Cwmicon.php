...
class Cwmicon
{
    ...
    public static function create($category, $params, $attribs = array(), $legacy = false)
    {
        ...
        $output = '<span class="hasTooltip" title="' . HtmlHelper::tooltipText(
                'JBS_CREATE_SERMON'
            ) . '">' . $button . '</span>';
    ...
    public static function edit($article, $params, $attribs = array(), $legacy = false)
    {
        ...
        if (property_exists($article, 'checked_out')
            ...
            $tooltip      = Text::_('JLIB_HTML_CHECKED_OUT') . ' :: ' . Text::sprintf(
                    'COM_CONTENT_CHECKED_OUT_BY',
                    $checkoutUser->name
                )
                . ' <br /> ' . $date;

            return '<span class="hasTooltip" title="' . HtmlHelper::tooltipText(
                    $tooltip . '',
                    0
                ) . '">' . $button . '</span>';
        }

        $url = 'index.php?option=com_proclaim&task=cwmmessageform.edit&a_id=' . $article->id . '&return=' . base64_encode(
                $uri
            );
        ...
        if ($legacy) {
            ...
            if (strtotime($article->publish_up) > strtotime(Factory::getDate())
                || ((strtotime($article->publish_down) < strtotime(
                            Factory::getDate()
            ...
            $text = '<span class="hasTooltip icon-' . $icon . ' tip" title="' . HtmlHelper::tooltipText(
                    Text::_('COM_CONTENT_EDIT_ITEM'),
                    $overlib,
                    0
                )
                . '"></span>'
