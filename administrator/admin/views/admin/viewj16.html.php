...
class biblestudyViewadmin extends JView {
    ...
    function display($tpl = null) {
        ...
        if (!$checkstories) {
            $studypath = JPATH_SITE . DS . 'images';
        }
        ...
        if ($fileList) {
            foreach ($fileList as $key => $value) {
                ...
                $folderfinal1->id = $key;
                if (strtolower($folderfinal1->value) == 'index.html') {
                    unset($folderfinal1->value);
                    unset($folderfinal1->key);
                } else {
