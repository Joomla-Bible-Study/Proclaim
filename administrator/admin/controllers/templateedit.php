...
class biblestudyControllertemplateedit extends controllerForm {
	...
	function unpublish(){
		$mainframe =& JFactory::getApplication();
		$cid 	= JRequest::getVar( 'cid', array(0), 'post', 'array' );
		if ($cid[0] == 1) {$msg = JText::_( 'JBS_TPL_ERROR_NO_UNPUBLISH_DEFAULT_TEMPLATE' );}
		else
		{
			if (!is_array( $cid ) || count( $cid ) < 1) {
	...
	function remove(){
		...
		if(!$model->delete()) {
			$cid 	= JRequest::getVar( 'cid', array(0), 'post', 'array' );
			if ($cid[0] == 1) {$msg = JText::_( 'JBS_TPL_ERROR_NO_DELETE_DEFAULT_TEMPLATE' );}
			else {$msg = JText::_( 'JBS_TPL_ERROR_DELETING_TEMPLATE' );}
		} else {
