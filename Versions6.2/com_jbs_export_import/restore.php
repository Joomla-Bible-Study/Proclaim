...
class JBSImport {
    ...
    function _getPackageFromUpload() {
        ...
        if (!(bool) ini_get('file_uploads')) {
            JError::raiseWarning('SOME_ERROR_CODE', JText::_('FILE_UPLOADS_DISABLED'));
            return false;
        }

        // Make sure that zlib is loaded so that the package can be unpacked
        if (!extension_loaded('zlib')) {
            JError::raiseWarning('SOME_ERROR_CODE', JText::_('ZLIB_NOT_PRESENT'));
            return false;
        }
        ...
        if ($userfile['error'] || $userfile['size'] < 1) {
            JError::raiseWarning('SOME_ERROR_CODE', JText::_('UPLOAD_ERROR'));
            return false;
        }
