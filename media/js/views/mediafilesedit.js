...
$j(document).ready(function () {
	...
	$j('#articlesSections').change(function () {
		...
		var url = 'index.php?option=com_proclaim&controller=mediafilesedit&task=articlesSectionCategories&format=raw&secId=' +
			secId
		if (secId !== '')
		{
			$j.ajax({
				...
				success: function (data) {
					$j.each(data, function (entryIndex, entry) {
						$j('#articleSectionCategories').
							addOption(entry['id'], entry['title'])
	...
	$j('#articleSectionCategories').change(
		function () {
			$j('#categoryItems').removeOption(/./)
			var catId = $j('#articleSectionCategories option:selected').attr('value')
			var url = 'index.php?option=com_proclaim&controller=mediafilesedit&task=articlesCategoryItems&format=raw&catId=' +
				catId
			$j.ajax({
				dataType: 'json',
				url: url,
