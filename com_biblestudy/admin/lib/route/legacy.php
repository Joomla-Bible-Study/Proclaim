...
require_once JBSMPATH_SITE . '/router.php';

/**
 * Class KunenaRouteLegacy
 */
abstract class JBSMRouteLegacy
{
	...
	public static function convert($uri, $showstart = 1)
	{
		...
		switch ($uri->getVar('view'))
		{
			...
			case 'view' :
				...
				$id = $uri->getVar ( 'id' );
				$message = JBSMMessageHelper::get ( $id );
			...
			case 'profile' :
				...
				if ($uri->getVar('task'))
				{
					$app->enqueueMessage ( JText::_ ( 'COM_BIBLESTUDY_DEPRECATED_ACTION' ), 'error' );
					$uri->delVar ( 'task' );
				}
		...
		if (self::convert($uri, 0))
		{
			...
			switch ($view)
			{
				...
				case 'post' :
					...
					break;
				case 'rules' :
					$params->set('body', '[article=full]'.JFactory::getConfig()->get('rules_cid', 1).'[/article]');
					$params->set('body_format', 'bbcode');
					$params->set('do', null);
					break;
				case 'help' :
					$params->set('body', '[article=full]'.JFactory::getConfig()->get('help_cid', 1).'[/article]');
					$params->set('body_format', 'bbcode');
					$params->set('do', null);
					break;
			}
