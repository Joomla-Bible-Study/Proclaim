...
abstract class JBSMRoute
{
	...
	{
		JBSM_PROFILER ? JBSMProfiler::instance()->start('function '.__CLASS__.'::'.__FUNCTION__.'()') : null;
		$uri = self::prepare();
		...
		$result = $uri->getQuery ();
		JBSM_PROFILER ? JBSMProfiler::instance()->stop('function '.__CLASS__.'::'.__FUNCTION__.'()') : null;
	...
	public static function _($uri = null, $xhtml = true, $ssl=0)
	{
		...
		JBSM_PROFILER ? JBSMProfiler::instance()->start('function '.__CLASS__.'::'.__FUNCTION__.'(t)') : null;
		self::$uris[$key] = JRoute::_ ( 'index.php?' . $uri->getQuery (), $xhtml, $ssl ) . ($fragment ? '#'.$fragment : '');
		JBSM_PROFILER ? JBSMProfiler::instance()->stop('function '.__CLASS__.'::'.__FUNCTION__.'(t)') : null;
		self::$urisSave = true;
		JBSM_PROFILER ? JBSMProfiler::instance()->stop('function '.__CLASS__.'::'.__FUNCTION__.'()') : null;
	...
	public static function cacheLoad()
	{
		...
		if (!JConfig::getInstance()->get('cache_url'))
		{
			return;
		}

		JBSM_PROFILER ? JBSMProfiler::instance()->start('function '.__CLASS__.'::'.__FUNCTION__.'()') : null;
		$user = JBSMUserHelper::getMyself();
		$cache = self::getCache();
	...
	public static function resolveAlias($alias)
	{
		JBSM_PROFILER ? JBSMProfiler::instance()->start('function '.__CLASS__.'::'.__FUNCTION__.'()') : null;
		$db = JFactory::getDbo();
		$query = "SELECT * FROM `#__bsms_aliases` WHERE alias LIKE {$db->Quote($alias.'%')}";
		$db->setQuery ($query);
...
//		}
	...
	protected static function prepare($uri = null)
	{
		...
		elseif ($option && $option != 'com_biblestudy')
		{
			JBSM_PROFILER ? JBSMProfiler::instance()->stop('function '.__CLASS__.'::'.__FUNCTION__.'()') : null;
		...
		{
			$result = JBSMRouteLegacy::convert($uri);
			JBSM_PROFILER ? JBSMProfiler::instance()->stop('function '.__CLASS__.'::'.__FUNCTION__.'()') : null;
		...
		switch ($uri->getVar('view', 'home')) {
			case 'announcement':
				if ($legacy_urls) JBSMRouteLegacy::convert($uri);
				break;
	...
	protected static function build()
	{
		...
		if (self::$search === false)
		{
			...
			if (JConfig::getInstance()->get('cache_mid'))
			{
				// FIXME: Experimental caching.
				$cache = self::getCache();
