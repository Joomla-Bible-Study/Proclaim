 ...
 */
abstract class JBSMMessageHelper
{
	/**
	 * @var JBSMMessage[]
	 */
	 ...
	 * @param bool $reload
	 *
	 * @return JBSMMessage	The message object.
	 */
	static public function get($identifier = null, $reload = false)
	{
		if ($identifier instanceof JBSMMessage)
		{
			return $identifier;
		}
		...
		if ($id < 1)
		{
			return new JBSMMessage;
		}

		if (empty(self::$_instances[$id]))
		{
			$instance = new JBSMMessage;
	...
	{
		$topic = JBSMTopicHelper::get($topic);
		if (!$topic->exists())
		{
			return array();
		}
	...
	static public function getLatestMessages($categories = false, $limitstart = 0, $limit = 0, $params = array())
	{
		...
		$categories = JBSMCategoryHelper::getCategories($categories, $reverse, 'topic.'.$authorise);

		if ($childforums)
		{
			$categories += JBSMCategoryHelper::getChildren($categories, -1, array('action'=>'topic.'.$authorise));
		}
		...
		{
			$instance = new JBSMMessage ($result);
	...
	public static function getLocation($mesid, $direction = null, $hold = null)
	{
		...
		if (!$hold)
		{
			...
			$mes_instance = JBSMMessageHelper::get($mesid);
	...
	static protected function loadMessages(array $ids)
	{
		...
		foreach ($ids as $id)
		{
			if (isset($results[$id]))
			{
				$instance = new JBSMMessage ($results[$id]);
				$instance->exists(true);
				self::$_instances [$id] = $instance;
			}
			else
	...
	static protected function loadMessagesByTopic($topic_id, $start = 0, $limit = 0, $ordering = 'ASC', $hold=0, $orderbyid = false)
	{
		...
		$location = ($orderbyid || $ordering == 'ASC') ? $start : JBSMTopicHelper::get($topic_id)->getTotal($hold) - $start - 1;
