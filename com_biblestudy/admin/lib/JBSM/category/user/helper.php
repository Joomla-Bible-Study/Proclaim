...
/**
 * Class JBSMCategoryUserHelper
 */
abstract class JBSMCategoryUserHelper
{
	// Global for every instance
	protected static $_instances = array();

	/**
	 * Get an instance of JBSMCategoryUser object.
	 *
	 ...
	 *
	 * @return JBSMCategoryUser	The user category object.
	 */
	static public function get($category = null, $user = null, $reload = false)
	{
		if ($category instanceof JBSMCategory)
		{
		...
		if ($category === null)
		{
			return new JBSMCategoryUser (null, $user);
		}

		if ($reload || empty ( self::$_instances [$user->userid][$category] ))
		{
			$user_categories = JBSMCategoryUserHelper::getCategories ( $category, $user );
	...
	static public function getCategories($ids = false, $user = null)
	{
		...
		if ($ids === false)
		{
			...
			$ids = JBSMCategoryHelper::getCategories();
		}
		elseif (!is_array ($ids) )
		{
			$ids = array($ids);
		...
		foreach ($ids as $i => $id)
		{
			if ($id instanceof JBSMCategory) $ids[$i] = $id->id;
		}
	...
	static public function markRead(array $ids, $user = null)
	{
		...
		$items = JBSMCategoryUserHelper::getCategories($ids, $user);
		$updateList = array();
		$insertList = array();
	...
	static protected function loadCategories(array $ids, KunenaUser $user)
	{
		...
		foreach ( $ids as $id )
		{
			...
			{
				self::$_instances [$user->userid][$id] = new JBSMCategoryUser ($id, $user);
			}
		}
