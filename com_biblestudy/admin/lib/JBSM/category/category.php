 ...
 */
class JBSMCategory extends KunenaDatabaseObject
{
	public $id = null;
	public $level = 0;
	...
	*/
	...
	/**
	* Returns the global JBSMCategory object.
	*
	...
	*/
	...
	{
		return JBSMCategoryHelper::get($identifier, $reload);
	}
	 ...
	 *
	 * @return array	List of JBSMCategory objects.
	 */
	public function getChildren($levels = 0)
	{
		return JBSMCategoryHelper::getChildren($this->id, $levels);
	}
	 ...
	 * @param mixed $user
	 *
	 * @return JBSMCategoryUser
	 */
	public function getUserInfo($user = null)
	{
		return JBSMCategoryUserHelper::get($this->id, $user);
	}
	...
	{
		$usercategory = JBSMCategoryUserHelper::get($this->id, $user);
	...
	{
		$category = $category ? JBSMCategoryHelper::get($category) : $this;
	...
	public function getUri($category = null, $action = null)
	{
		...
		{
			$category = JBSMCategoryHelper::get($category);
		}
	...
	{
		return JBSMTopicHelper::get($this->getLastCategory()->last_topic_id);
	}
		...
		if ($this->_channels === false)
		{
			...
			else
			{
				...
				if (!empty($ids))
				{
					...
					$this->_channels['none'] += JBSMCategoryHelper::getCategories(array_keys($ids), null, 'none');
				}
				if (isset($ids['CHILDREN']))
				{
					// Children category channels
					$this->_channels['none'] += JBSMCategoryHelper::getChildren($this->id, 1, array($action=>'none'));
				}
			}
		}
	...
	public function getNewTopicCategory($user = null)
	{
		...
		$categories = JBSMCategoryHelper::getChildren(intval($this->id), -1, array('action'=>'topic.create'));
	...
	{
		$parent = JBSMCategoryHelper::get(intval($this->parent_id));
	...
	public function load($id = null)
	{
		...
		if ($exists)
		{
			JBSMCategoryHelper::register($this);
		}
	...
	public function purge($time, $params = array(), $limit = 1000)
	{
		...
		$count = JBSMTopicHelper::delete($ids);

		KunenaUserHelper::recount();
		JBSMCategoryHelper::recount($this->id);
	...
	public function trash($time, $params = array(), $limit = 1000)
	{
		...
		$count = JBSMTopicHelper::trash($ids);
	...
	/**
	 * Method to check out the JBSMCategory object.
	 *
	 * @param	integer	$who
	 *
	...
	/**
	 * Method to check in the JBSMCategory object.
	 *
	...
	public function getSubscribed($userid = null )
	{
		...
		$usercategory = JBSMCategoryUserHelper::get($this->id,$userid);
	...
	protected function buildInfo()
	{
		...
		$this->_posts = 0;
		$this->_lastcat = $this;
		/** @var array|JBSMCategory[] $categories */
		$categories[$this->id] = $this;
		// TODO: support channels
		//$categories += $this->getChannels();
		$categories += JBSMCategoryHelper::getChildren($this->id);
