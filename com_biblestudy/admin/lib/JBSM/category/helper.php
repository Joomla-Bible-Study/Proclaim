...
/**
 * Class JBSMCategoryHelper
 */
abstract class JBSMCategoryHelper
{
	/**
	 * @var JBSMCategory[]
	 */
	...
	static public function initialize()
	{
		...
		if (KunenaConfig::getInstance()->get('cache_cat'))
		{
			...
			self::$_instances = $cache->call(array('JBSMCategoryHelper', 'loadCategories'));
		}
		else
		{
	...
	/**
	 * Returns the global JBSMCategory object, only creating it if it doesn't already exist.
	 *
	 * @param int  $identifier	The category to load - Can be only an integer.
	 * @param bool $reload		Reload category from the database.
	 *
	 * @return JBSMCategory	The Category object.
	 *
	 * @since	1.6
	 */
	static public function get($identifier = null, $reload = false)
	{
		...
		if ($identifier instanceof JBSMCategory)
		{
			KUNENA_PROFILER ? KunenaProfiler::instance()->stop('function '.__CLASS__.'::'.__FUNCTION__.'()') : null;
		...
		if (empty ( self::$_instances [$id] )) {
			self::$_instances [$id] = new JBSMCategory (array('id'=>$id));
	...
	static public function getSubscriptions($user = null)
	{
		...
		return JBSMCategoryHelper::getCategories($subscribed);
	}
	...
	public static function subscribe($ids, $value = true, $user = null)
	{
		$count = 0;
		// Pre-load all items
		$usercategories = JBSMCategoryUserHelper::getCategories($ids, $user);
	...
	static protected function _getChildren(array $parents, $levels, array $params, $optimize)
	{
		...
		{
			if ($parent instanceof JBSMCategory)
			{
				$parent = $parent->id;
			}
	...
	static public function &loadCategories()
	{
		...
		$db->setQuery ( $query );
		$instances = (array) $db->loadObjectList('id', 'JBSMCategory');
