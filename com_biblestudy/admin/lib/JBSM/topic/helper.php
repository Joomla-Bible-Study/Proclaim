...
/**
 * Class JBSMTopicHelper
 */
abstract class JBSMTopicHelper
{
	/**
	 * @var JBSMTopic[]
	 */
	...
	{
		if ($identifier instanceof JBSMTopic)
		{
			return $identifier;
		}
		...
		if ($id < 1)
		{
			return new JBSMTopic;
		}

		if (empty(self::$_instances[$id]))
		{
			$instance = new JBSMTopic;
	...
	{
		// Pre-load all items
		JBSMTopicUserHelper::getTopics($ids, $user);
		$count = 0;
		foreach ($ids as $id)
		{
			$usertopic = JBSMTopicUserHelper::get($id, $user);
	...
	static public function getUserTopics($ids = false, $user = null)
	{
		...
		return JBSMTopicUserHelper::getTopics($ids, $user);
	}
	...
	static public function getLatestTopics($categories=false, $limitstart=0, $limit=0, $params=array())
	{
		...
		$categories = JBSMCategoryHelper::getCategories($categories, $reverse);
		$catlist = array();
	...
	static protected function loadTopics(array $ids)
	{
		...
		foreach ( $ids as $id )
		{
			if (isset($results[$id]))
			{
				$instance = new JBSMTopic ($results[$id]);
 				$instance->exists(true);
 				self::$_instances [$id] = $instance;
			}
			else
