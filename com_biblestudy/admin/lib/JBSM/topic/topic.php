 ...
 */
class JBSMTopic extends KunenaDatabaseObject
{
	/**
	 * @var int
	 */
	...
	{
		return JBSMTopicHelper::get($identifier, $reset);
	}
	...
	public function sendNotification($url = null)
	{
		...
		JBSMMessageHelper::get($this->first_post_id, true)->sendNotification($url);
	}

	/**
	 * @param mixed $user
	 *
	 * @return JBSMTopicUser
	 */
	public function getUserTopic($user = null)
	{
		$usertopic = JBSMTopicUserHelper::get($this, $user);
	...
	{
		return JBSMCategoryHelper::get($this->category_id);
	}
	...
	{
		$poll = JBSMTopicPollHelper::get($this->poll_id);
	...
	{
		return JBSMTopicUserHelper::get($this->id, $user);
	}
	 ...
	 *
	 * @return  JBSMTopic  Returns this topic or move target if this was moved.
	 *
	...
	public function getTopic()
	{
		...
		while ($topic->moved_id)
		{
			...
			$topic = JBSMTopicHelper::get($topic->moved_id);
		}
	...
	public function getTotal($hold = null)
	{
		...
		return JBSMMessageHelper::getLocation($this->last_post_id, 'both', $hold) + 1;
	}
	...
	public function getUrl($category = null, $xhtml = true, $action = null)
	{
		return KunenaRoute::getTopicUrl($this, $xhtml, $action,
			$category ? JBSMCategoryHelper::get($category) : $this->getCategory());
	}
	...
	{
		$category = $category ? JBSMCategoryHelper::get($category) : $this->getCategory();
		...
		if ($action instanceof JBSMMessage)
		{
			$message = $action;
	...
	public function getPostLocation($mesid, $direction = null, $hold = null)
	{
		...
		return JBSMMessageHelper::getLocation($mesid, $direction, $hold);
	}
	...
	public function hasNew($user = null)
	{
		...
		$userinfo = JBSMCategoryUserHelper::get($this->getCategory(), $user);
		...
		$read = JBSMTopicUserReadHelper::get($this, $user);
	...
	/**
	 * Method to load a JBSMTopic object by id.
	 *
	 ...
	 *
	 * @param   object  $target        Target JBSMCategory or JBSMTopic
	 ...
	 *
	 * @return 	bool|JBSMCategory|JBSMTopic	Target JBSMCategory or JBSMTopic or false on failure
	 */
	public function move($target, $ids = false, $shadow = false, $subject = '', $subjectall = false, $topic_iconid = null)
	{
		...
		elseif ($target instanceof JBSMTopic)
		{
		...
		elseif ($target instanceof JBSMCategory)
		{
		...
		if (!$ids && !empty($categoryTarget))
		{
			...
			JBSMTopicUserHelper::move($this, $target);
			// TODO: do we need this?
			//JBSMTopicUserReadHelper::move($this, $target);
		...
		elseif (!$ids)
		{
			...
			// Update user topic information (topic, category)
			JBSMTopicUserHelper::merge($this, $target);
			// TODO: do we need this?
			//JBSMTopicUserReadHelper::merge($this, $target);
			// Remove topic and posts from the old category
	...
	/**
	 * Method to save the JBSMTopic object to the database.
	 *
	 * @param bool $cascade
	 *
	 * @return bool	True on success.
	...
	/**
	 * Method to put the JBSMTopic object on trash this is still present in database.
	 *
	 * @return bool	True on success.
	 */
	...
	/**
	 * Method to delete the JBSMTopic object from the database.
	 *
	 * @param bool $recount
	 *
	 * @return bool	True on success.
	...
	public function delete($recount = true)
	{
		...
		if ($this->moved_id == 0)
		{
			...
			if ($recount)
			{
				...
				JBSMCategoryHelper::recount();
				KunenaAttachmentHelper::cleanup();
				JBSMMessageThankyouHelper::recount();
			}
		}
	...
	public function update($message = null, $postdelta = 0)
	{
		...
		{
			JBSMTopicUserHelper::recount($this->id);
			// FIXME: optimize
			KunenaUserHelper::recount();
		}
