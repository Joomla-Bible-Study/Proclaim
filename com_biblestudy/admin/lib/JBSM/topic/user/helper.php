 ...
 */
abstract class JBSMTopicUserHelper
{
	/**
	 * @var array|JBSMTopicUser[]
	 */
	...
	/**
	 * Returns JBSMTopicUser object.
	 *
	 * @param JBSMTopic|int|null $topic
	 * @param mixed $user
	 * @param bool $reload
	 *
	...
	static public function get($topic = null, $user = null, $reload = false)
	{
		...
		if ($topic < 1)
		{
			return new JBSMTopicUser (null, $user);
		}
	...
	static public function getTopics($ids = false, $user = null)
	{
		...
		foreach ($ids as $i => $id)
		{
			if ($id instanceof JBSMTopic) $ids[$i] = $id->id;
		}
	...
	static public function getUserIds(array $topics, $value = 'user_id')
	{
		...
		{
			if ($id instanceof JBSMTopic) $ids[(int) $id->id] = (int) $id->id;
	...
	static protected function loadTopics(array $ids, KunenaUser $user)
	{
		...
		foreach ( $ids as $id )
		{
			...
			{
				self::$_instances [$user->userid][$id] = self::$_topics [$id][$user->userid] = new JBSMTopicUser ($id, $user->userid);
			}
		}
