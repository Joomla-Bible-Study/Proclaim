...
/**
 * Class JBSMTopicUserReadHelper
 */
abstract class JBSMTopicUserReadHelper
{
	/**
	 * @var array|JBSMTopicUserRead[]
	 */
	...
	/**
	 * Returns JBSMTopicUserRead object.
	 *
	...
	static public function get($topic = null, $user = null, $reload = false)
	{
		...
		if ($topic < 1)
		{
			return new JBSMTopicUserRead (null, $user);
		}

		if (!$user->userid)
		{
			return new JBSMTopicUserRead ($topic, 0);
		}
	...
	static protected function loadTopics(array $ids, KunenaUser $user)
	{
		...
		foreach ( $ids as $id )
		{
			if (isset($results[$id]))
			{
				$instance = new JBSMTopicUserRead ();
			...
			{
				self::$_instances [$user->userid][$id] = self::$_topics [$id][$user->userid] = new JBSMTopicUserRead ($id, $user->userid);
			}
		}
