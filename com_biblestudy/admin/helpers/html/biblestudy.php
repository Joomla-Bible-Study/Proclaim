...
abstract class JHtmlBiblestudy
{
    ...
    public static function Mimetypelist()
    {
        ...
        try
        {
            $options = $db->loadObjectList(); dump($options);
        }
        catch (RuntimeException $e)
        {
            JError::raiseWarning(500, $e->getMessage());
    ...
    public static function link_type()
    {
        // Create the batch selector to change the player on a selection list.
        $lines = array(
            '<label id="batch-client-lbl" for="batch-client" class="hasTip" title="'.JText::_('JBS_MED_SHOW_DOWNLOAD_ICON').'::'.JText::_('JBS_MED_SHOW_DOWNLOAD_ICON_DESC').'">',
            JText::_('JBS_MED_SHOW_DOWNLOAD_ICON'),
            '</label>',
            '<select name="batch[link_type]" class="inputbox" id="batch-link_type">',
            '<option value="">'.JText::_('JBS_CMN_DOWNLOAD_NOCHANGE').'</option>',
            JHtml::_('select.options', self::Link_typelist(), 'value', 'text'),
            '</select>'
        );
    ...
    public static function mediatype()
    {
        // Create the batch selector to change the player on a selection list.
        $lines = array(
            '<label id="batch-client-lbl" for="batch-client" class="hasTip" title="'.JText::_('JBS_MED_IMAGE').'::'.JText::_('JBS_MED_IMAGE_DESC').'">',
            JText::_('JBS_MED_SELECT_MEDIA_TYPE'),
            '</label>',
