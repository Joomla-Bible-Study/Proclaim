...
class JBSMDbHelper
{
		...
		foreach ($tables as $t) {
			...
			switch ($type) {
				case 'drop':
					...
					if (JBSMDbHelper::checkTables($table, $field) === TRUE) {
						$query = 'ALTER TABLE `' . $table . '` DROP `' . $field . '`';
						if (!JBSMDbHelper::performDB($query, $from)) {
							return FALSE;
						}
					}
					break;
	...
	{
		$done = JBSMDbHelper::checkTables($table, $field);
		if (!$done) {
	...
	public static function fixupcss($filename, $parent, $newcss, $id)
	{
		...
		$db->setQuery($query);
		if (!$db->execute()) {
			throw new Exception(JText::sprintf('JBS_INS_SQL_UPDATE_ERRORS', $db->stderr(true)), 500);
			return FALSE;
		}
	...
	public static function resetdb()
	{
		...
		foreach ($files as $value) {
			...
			if ($buffer === false) {
				throw new Exception(JText::_('JBS_INS_ERROR_SQL_READBUFFER'), 500);
				return FALSE;
			}
		...
		throw new Exception(JText::_('JBS_INS_RESETDB'), 1);
		return TRUE;
	}
