...
function installSampleData()
{
	...
	$query = "INSERT INTO `#__bsms_aliases` (`alias`, `type`, `item`, `state`) VALUES
	...
	$queries[] = array ('bsms_aliases', $query);

	$query = "INSERT INTO `#__bsms_ranks`
	(`rank_id`, `rank_title`, `rank_min`, `rank_special`, `rank_image`) VALUES
	(1, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK1'))}, 0, 0, 'rank1.gif'),
	(2, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK2'))}, 20, 0, 'rank2.gif'),
	(3, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK3'))}, 40, 0, 'rank3.gif'),
	(4, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK4'))}, 80, 0, 'rank4.gif'),
	(5, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK5'))}, 160, 0, 'rank5.gif'),
	(6, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK6'))}, 320, 0, 'rank6.gif'),
	(7, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK_ADMIN'))}, 0, 1, 'rankadmin.gif'),
	(8, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK_MODERATOR'))}, 0, 1, 'rankmod.gif'),
	(9, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK_SPAMMER'))}, 0, 1, 'rankspammer.gif'),
	(10, {$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_RANK_BANNED'))}, 0, 1, 'rankbanned.gif');";

	$queries[] = array ('bsms_ranks', $query);

	$query = "INSERT INTO `#__bsms_smileys`
	...
	$queries[] = array ('bsms_smileys', $query);

	$section = KText::_('COM_BIBLESTUDY_SAMPLEDATA_SECTION_TITLE');
	$cat1 = KText::_('COM_BIBLESTUDY_SAMPLEDATA_CATEGORY1_TITLE');
	$cat2 = KText::_('COM_BIBLESTUDY_SAMPLEDATA_CATEGORY2_TITLE');
	$section_alias = BibleStudyRoute::stringURLSafe(KText::_('COM_BIBLESTUDY_SAMPLEDATA_SECTION_TITLE'), 'main-forum');
	$cat1_alias = BibleStudyRoute::stringURLSafe(KText::_('COM_BIBLESTUDY_SAMPLEDATA_CATEGORY1_TITLE'), 'welcome-mat');
	$cat2_alias = BibleStudyRoute::stringURLSafe(KText::_('COM_BIBLESTUDY_SAMPLEDATA_CATEGORY2_TITLE'), 'suggestion-box');

	$aliasquery = "INSERT INTO `#__bsms_aliases` (`alias`, `type`, `item`, `state`) VALUES
		({$db->quote($section_alias)}, 'catid', '1', 1),
		({$db->quote($cat1_alias)}, 'catid', '2', 1),
		({$db->quote($cat2_alias)}, 'catid', '3', 1);";

	$query="INSERT INTO `#__bsms_categories`
		...
		(1, 0, {$db->quote($section)}, {$db->quote($section_alias)}, 1, 1, 1, ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_SECTION_DESC')).", ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_SECTION_HEADER')).", 0, 0, 0, 0, 0, 0, 'joomla.group'),
		(2, 1, {$db->quote($cat1)}, {$db->quote($cat1_alias)}, 1, 1, 1, ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_CATEGORY1_DESC')).", ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_CATEGORY1_HEADER')).", 1 , 1, 0, 1, 1, {$posttime->toUnix()}, 'joomla.group'),
		(3, 1, {$db->quote($cat2)}, {$db->quote($cat2_alias)}, 1, 2, 1, ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_CATEGORY2_DESC')).", ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_CATEGORY2_HEADER')).", 0 , 0, 1, 0, 0, 0, 'joomla.group');";

	$queries[] = array ('bsms_categories', $query);

	$query="INSERT INTO `#__bsms_messages`
	(`id`, `parent`, `thread`, `catid`, `userid`, `name`, `subject`, `time`, `ip`) VALUES
	(1, 0, 1, 2, ".$db->quote($my->id).", 'Kunena', ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_POST_WELCOME_SUBJECT')).", ".$posttime->toUnix().", '127.0.0.1');";

	$queries[] = array ('bsms_messages', $query);

	$query="INSERT INTO `#__bsms_messages_text`
	(`mesid`, `message`) VALUES
	(1, ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_POST_WELCOME_TEXT_CONTENT')).");";

	$queries[] = array ('bsms_messages_text', $query);

	$query="INSERT INTO `#__bsms_topics`
	...
	(1, 2, ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_POST_WELCOME_SUBJECT')).", 1, 1, ".$posttime->toUnix().", ".$db->quote($my->id).", ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_POST_WELCOME_TEXT_CONTENT')).", 'Kunena', 1, ".$posttime->toUnix().", ".$db->quote($my->id).", ".$db->quote(KText::_('COM_BIBLESTUDY_SAMPLEDATA_POST_WELCOME_TEXT_CONTENT')).", 'Kunena');";

	$queries[] = array ('bsms_topics', $query);
	...
	foreach ($queries as $query)
	{
		...
		if (!$filled) {
			...
			$db->execute();
			if ($db->getErrorNum ())
				throw new BibleStudyInstallerException ( $db->getErrorMsg (), $db->getErrorNum () );
			if ($query[0] == 'bsms_categories') {
	...
	$query = "INSERT INTO `#__bsms_users` (userid, showOnline) SELECT a.id AS userid, 1 AS showOnline FROM `#__users` AS a LEFT JOIN `#__bsms_users` AS b ON b.userid=a.id WHERE b.userid IS NULL";
	$db->setQuery($query);
	$db->execute();
	if ($db->getErrorNum ())
		throw new BibleStudyInstallerException ( $db->getErrorMsg (), $db->getErrorNum () );
