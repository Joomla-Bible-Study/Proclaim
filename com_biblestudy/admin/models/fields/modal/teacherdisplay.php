...
class JFormFieldModal_Teacherdisplay extends JFormField
{
	...
	protected function getInput()
	{
		...
		$query->select('teachername AS name')->from('#__bsms_teachers')->where('id = ' . (int) $this->value);
		$db->setQuery($query);
		$title = $db->loadResult();
		...
		$html .= '"  href="' . $link . '" rel="{handler: \'iframe\', size: {x: 800, y: 450}}">' . JText::_('JBS_CMN_SELECT_TEACHER');
		$html .= '</a></div></div>' . "\n";
