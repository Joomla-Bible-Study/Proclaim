...
class BiblestudyModelMediafile extends JModelAdmin
{
    ...
    public function getItem($pk = null)
    {
        ...
        if (!empty($this->data)) {
            ...
            if ($server_id = $this->getState('mediafile.server.id')) {
                $this->data->server_id = $server_id;
            }

            $this->data->server = JModelLegacy::getInstance('Server', 'BiblestudyModel')->getItem($server_id);
            $this->data->config = static::getConfig($this->data->server->type);
        }

        return $this->data;
    }

    public static function getConfig($addon) {
        $path = JPATH_ADMINISTRATOR . '/components/com_biblestudy/addons/servers/' . $addon . '/media.xml';
    ...
    protected function populateState()
    {
        ...
        $this->setState('mediafile.server.id', $app->getUserState('com_biblestudy.edit.mediafile.server.id'));
    }
