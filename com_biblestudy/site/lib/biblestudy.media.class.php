...
class jbsMedia
{
	...
	{
		$db = JFactory::getDBO();
		$query = 'UPDATE #__bsms_mediafiles SET plays = plays + 1 WHERE id = ' . (int)$db->q($id);
		$db->setQuery('UPDATE ' . $db->qn('#__bsms_mediafiles') . 'SET ' . $db->qn('plays') . ' = ' . $db->qn('plays') . ' + 1 ' . ' 	WHERE id = ' . (int)$db->q($id));
		$db->query();
