...
JLoader::register('JBSListing', BIBLESTUDY_PATH_LIB . '/biblestudy.listing.class.php');
...
class JBSMListing
{
	...
	public static function getListing($row, $params, $oddeven, $admin_params, $template, $ismodule)
	{
		...
		{
			$elementid             = JBSMCustom::getCustom($params->get('row1col1'), $params->get('r1c1custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 1 && $params->get('r1c1span') < 2)
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row1col2'), $params->get('r1c2custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 2 && ($params->get('r1c1span') < 3 && $params->get('r1c2span') < 2))
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row1col3'), $params->get('r1c3custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 3 && ($params->get('r1c1span') < 4 && $params->get('r1c2span') < 3 && $params->get('r1c3span') < 2))
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row1col4'), $params->get('r1c4custom'), $row, $params, $admin_params, $template);
		...
		{
			$elementid             = JBSMCustom::getCustom($params->get('row2col1'), $params->get('r2c1custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 1 && $params->get('r2c1span') < 2)
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row2col2'), $params->get('r2c2custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 2 && ($params->get('r2c1span') < 3 && $params->get('r2c2span') < 2))
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row2col3'), $params->get('r2c3custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 3 && ($params->get('r2c1span') < 4 && $params->get('r2c2span') < 3 && $params->get('r2c3span') < 2))
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row2col4'), $params->get('r2c4custom'), $row, $params, $admin_params, $template);
		...
		{
			$elementid             = JBSMCustom::getCustom($params->get('row3col1'), $params->get('r3c1custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 1 && $params->get('r3c1span') < 2)
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row3col2'), $params->get('r3c2custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 2 && ($params->get('r3c1span') < 3 && $params->get('r3c2span') < 2))
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row3col3'), $params->get('r3c3custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 3 && ($params->get('r3c1span') < 4 && $params->get('r3c2span') < 3 && $params->get('r3c3span') < 2))
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row3col4'), $params->get('r3c4custom'), $row, $params, $admin_params, $template);
		...
		{
			$elementid             = JBSMCustom::getCustom($params->get('row4col1'), $params->get('r4c1custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 1 && $params->get('r4c1span') < 2)
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row4col2'), $params->get('r4c2custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 2 && ($params->get('r4c1span') < 3 && $params->get('r4c2span') < 2))
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row4col3'), $params->get('r4c3custom'), $row, $params, $admin_params, $template);
		...
		if ($columns > 3 && ($params->get('r4c1span') < 4 && $params->get('r4c2span') < 3 && $params->get('r4c3span') < 2))
		{
			...
			{
				$elementid             = JBSMCustom::getCustom($params->get('row4col4'), $params->get('r4c4custom'), $row, $params, $admin_params, $template);
	...
	private static function getLink($islink, $id3, $tid, $smenu, $tmenu, $params, $admin_params, $row, $template)
	{
		...
		switch ($islink)
		{
			...
			case 2 :
				$filepath = JBSMElements::getFilepath($id3, 'study_id', $mime);
	...
	public static function getListingExp($row, $params, $admin_params, $template)
	{
		...
		$label  = str_replace('{{date}}', JBSMElements::getStudydate($params, $row->studydate), $label);
		$label  = str_replace('{{studyintro}}', $row->studyintro, $label);
		$label  = str_replace('{{scripture}}', JBSMElements::getScripture($params, $row, 0, 1), $label);
		...
		$label  = str_replace('{{mediatime}}', JBSMElements::getDuration($params, $row), $label);
		...
		else
		{
			...
			foreach ($rows as $sharerow)
			{
				...
				if ($share_params->get('item1'))
				{
					...
					{
						$element           = JBSMCustom::getCustom($share_params->get('item1'), $share_params->get('item1custom'), $row, $params, $admin_params, $template = 1);
						$element1->element = $element->element;
					}
					else
					{
						$element1 = JBSMElements::getElementid($share_params->get('item1'), $row, $params, $admin_params, $template = 1);
					}
				}
				if ($share_params->get('item2'))
				{
					...
					{
						$element           = JBSMCustom::getCustom($share_params->get('item2'), $share_params->get('item2custom'), $row, $params, $admin_params, $template = 1);
						$element2->element = $element->element;
					}
					else
					{
						$element2 = JBSMElements::getElementid($share_params->get('item2'), $row, $params, $admin_params, $template = 1);
					}
				}
				if ($share_params->get('item3'))
				{
					...
					{
						$element           = JBSMCustom::getCustom($share_params->get('item3'), $share_params->get('item3custom'), $row, $params, $admin_params, $template = 1);
						$element3->element = $element->element;
					}
					else
					{
						$element3 = JBSMElements::getElementid($share_params->get('item3'), $row, $params, $admin_params, $template = 1);
					}
				}
				if ($share_params->get('item4'))
				{
					...
					{
						$element           = JBSMCustom::getCustom($share_params->get('item4'), $share_params->get('item4custom'), $row, $params, $admin_params, $template = 1);
						$element4->element = $element->element;
					}
					else
					{
						$element4 = JBSMElements::getElementid($share_params->get('item4'), $row, $params, $admin_params, $template = 1);
					}
				}
	...
	function getPassage($params, $row)
	{
		$esv          = 1;
		$scripturerow = 1;
		$scripture    = JBSMElements::getScripture($params, $row, $esv, $scripturerow);
