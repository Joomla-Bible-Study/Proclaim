...
$widpos1 = $params->get('widthcol1');
$widpos2 = $params->get('widthcol2');
$widpos3 = $params->get('widthcol3');
$widpos4 = $params->get('widthcol4');
...
if ($message) {?>
<tr><td align="center"><?php echo '<h2>'.$message.'</h2>';
//if ($this->params->get('study_publish') < 1){echo JText::_('Submissions must be approved prior to publication').'<br>';}?></td></tr><?php } //End of if $message
?>

</td></tr><?php //} //End of if $message?>
<tr><td><strong><?php echo JText::_('Studies');?></strong></td></tr>
<tr><td><a href="<?php echo JURI::base()?>index.php?option=com_biblestudy&controller=studiesedit&view=studiesedit&layout=form"><?php echo JText::_('Add a New Study');?></a></td></tr>
<tr><td><a href="<?php echo JURI::base()?>index.php?option=com_biblestudy&controller=mediafilesedit&view=mediafilesedit&layout=form"><?php echo JText::_('Add a New Media File Record');?></a></td></tr>
<?php if ($params->get('show_comments') > 0){?>
<tr><td><a href="<?php echo JURI::base()?>index.php?option=com_biblestudy&view=commentslist"><?php echo JText::_('Manage Comments');?></a></td></tr><?php } //end if show_comments?>
...
?></strong><tr><td><?php //This is a row to hold the podcast listings
?><a href="<?php echo JURI::base().'index.php?option=com_biblestudy&controller=podcastedit&view=podcastedit&layout=form';?>"><?php echo JText::_('Add A Podcast');?></a></td></tr>
<tr><td><?php foreach ($podcasts as $podcast) { $pod = $podcast['id']; $podtitle = $podcast['title'];
?><tr><td><a href="<?php echo JURI::base().'index.php?option=com_biblestudy&controller=podcastedit&view=podcastedit&layout=form&task=edit&cid[]='.$pod;?>"><?php echo $podtitle;?></a></td></tr>
<?php } // end foreach for podcasts as podcast
// End row for podcast?> </td></tr>
<?php
...
<?php if ( $this->params->get( 'show_page_title_list' ) >0 ) { ?>
            ...
            </table><?php //End of table for logo?>
            
         
                
 			</td></tr><?php //end of row above drop down boxes?>
            <?php if ($this->params->get('show_teacher_list') >0) { ?>
		<tr><td cellpadding="2" width ="<?php echo $this->params->get('teacherw');?>"><img src="<?php echo $this->params->get('teacherimage');?>" border="1" width="<?php echo $this->params->get('teacherw');?>" height="<?php echo $this->params->get('teacherh');?>" /><br /><?php echo $this->params->get('teachername');?></td></tr>
		<?php }?>
            <tr><td><?php //Row for drop down boxes?>
            ...
            <?php //This is the column that holds the search drop downs?>
                    ...
                    <?php } ?>
                    <?php if ($this->params->get('show_order_search') >0) { ?>
                    <?php
                    ...
                    <?php } ?>
                    <?php if ($this->params->get('show_topic_search') >0) { ?>
        	...
        	<table <?php if ($color > 0){echo 'bgcolor="'.$bgcolor.'"';}?> width="<?php echo $page_width; ?>" cellpadding="0" cellspacing="0"><?php //6 Column table?>
            	<tr valign="<?php echo $params->get('colalign');?>"><?php //Row for 6 column table?>
                   ...
                   <?php //Remove all rows in the array that are not Column1
                        ...
                        <table  border="<?php echo $params->get('border');?>" cellpadding="<?php echo $params->get('padding');?>" cellspacing="<?php echo $params->get('spacing');?>"> <?php //This is the table for the list. It's outside the foreach?>
							...
							//Now we produce each element in turn with its parameters
								...
								echo '</span>'; 
							?>
                            </td></tr> <?php //This is tne end of each td and row for the list ?>
                            <?php
							} //End of foreach Column1
							?>
							</table> <?php //This ends the table inside of column 1 that holds the actual listings. ?>

                           
                                
                        </td><?php }//End of column 1 of 5 Also end of if ($column1->position)?>
                        ...
                        <td width="<?php echo $widpos2;?>"><?php //Beginning of Column 2 of 5?>
							...
							?>
							</table> <?php //This ends the table inside of column 2 that holds the actual listings. It is outside the foreach loop?>
                            
                                
                        </td><?php }//End of Column 2 of 5 And if($column2->position?>
                        ...
                        <td width="<?php echo $widpos3;?>"><?php //Begin Column 3 of 5?>
							...
							?>
							</table> <?php //This ends the table inside of column 3 that holds the actual listings. It is outside the foreach loop?>
                            
                        </td><?php }//End of Column 3 of 5 and $jf($column3->position)?>
                        ...
                        <td width="<?php echo $widpos4;?>"><?php //Begin Column 4 of 5?>
							...
							?>
							</table> <?php //This ends the table inside of column 4 that holds the actual listings. It is outside the foreach loop?>
                            
                        </td><?php }//End of Column 4 of 5 and if($column4->position)?>
                    
                    <?php if (($params->get('show_full_text') + $params->get('show_pdf_text')) > 0) { //Tests to see if show text and/or pdf is set to "show"?>    
                   ...
                   <?php if ($params->get('show_store') > 0){?>
						...
						if(list($width,$height)=@getimagesize($src)){}
						else {$width = 24; $height= 24;}?><td><?php echo '<a href="'.$cd->server_path.$cd->prod_cd.'" title="'.$cd->media_alttext.'"><img src="'.JURI::base().$cd->media_image_path.'" width="'.$width.'" height="'.$height.'" alt="'.$cd->media_alttext.' "border="0"></a>';?></td><?php } ?>
                        <?php if ($dvd->mid > 0){ 
						$src = JURI::base().$dvd->media_image_path;
						if(list($width,$height)=@getimagesize($src)){}
						else {$width = 24; $height= 24;}?><td><?php echo '<a href="'.$dvd->server_path.$dvd->prod_dvd.'" title="'.$dvd->media_alttext.'"><img src="'.JURI::base().$dvd->media_image_path.'" width="'.$width.'" height="'.$height.'" alt="'.$dvd->media_alttext.' "border="0"></a>';?></td><?php } ?>
...
<param name="wmode" value="transparent">
                    ...
                    </tr></table><?php //This ends the table that holds the media images and is outside the foreach?>
                    </td><?php //End column 6 of 6?>
                    
                    <?php } //This is the end of the if show media statement ?>
                    
                </tr><?php //End row for 6 column table?>
            </table><?php //End 6 Column table?>       
<table width="<?php echo $page_width; ?>" <?php if ($color > 0){echo 'bgcolor="'.$bgcolor.'"';}?>>
   			<?php if ($show_description > 0) { ?>
   			<tr><td>
            <?php  echo '<span '.$params->get('descriptionspan').'> '.$row->studyintro.'</span>'; ?> 
            </td></tr>
 			<?php if ($params->get('line') > 0) { ?>
            <tr><td width="<?php echo $params->get('mod_table_width');?>"><?php //This row is to hold the line and should run along the bottom of the 5 column table?>
            	<?php echo '<img src="'.JURI::base().'components/com_biblestudy/images/square.gif" height="2" width="100%">'; ?>
                <?php } //End of if show lines?></td></tr>
...
<?php	  	$row_count++; // This increments the row count and adjusts the variable for the color background
		$k = 3 - $k; ?>
<?php } //This is the end of the for statement for each result from the database that will create its own 6 column table?>
        
       		</td></tr><?php //End of row for 6 column table?>
