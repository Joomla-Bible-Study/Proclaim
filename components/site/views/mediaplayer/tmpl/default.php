...
$booknumber = $this->mediaplayer->booknumber;
$ch_b = $this->mediaplayer->chapter_begin;
$ch_e = $this->mediaplayer->chapter_end;
$v_b = $this->mediaplayer->verse_begin;
$v_e = $this->mediaplayer->verse_end;
$scripture1 = format_scripture($booknumber, $ch_b, $ch_e, $v_b, $v_e);

$booknumber = $this->mediaplayer->booknumber2;
$ch_b = $this->mediaplayer->chapter_begin2;
$ch_e = $this->mediaplayer->chapter_end2;
$v_b = $this->mediaplayer->verse_begin2;
$v_e = $this->mediaplayer->verse_end2;
...
$duration = $this->mediaplayer->media_hours.$this->mediaplayer->media_minutes.$this->mediaplayer->media_seconds;
	...
	switch ($duration_type) {
		case 1: 
			...
			if (!$duration){
				...
				if (!$this->mediaplayer->media_hours){
					$duration = $this->mediaplayer->media_minutes.' mins '.$this->mediaplayer->media_seconds.' secs';
				}
				else {
					$duration = $this->mediaplayer->media_hours.' hour(s) '.$this->mediaplayer->media_minutes.' mins '.$this->mediaplayer->media_seconds.' secs';
				}
			}
			break;
		case 2: 
			...
			if (!$duration){
				...
				if (!$this->mediaplayer->media_hours){
					$duration = $this->mediaplayer->media_minutes.':'.$this->mediaplayer->media_seconds;
				}
				else {
					$duration = $this->mediaplayer->media_hours.':'.$this->mediaplayer->media_minutes.':'.$this->mediaplayer->media_seconds;
				}
...
<table class="contentpaneopen<?php echo $this->params->get( 'pageclass_sfx' ); ?>">
	<tr>
		...
		<?php echo '<span '.$this->params->get('style_title_view').'>'.$this->mediaplayer->studytitle.'</span>'; ?>
		<?php endif; ?>
	</td>	
	</tr>
	...
	<tr>
		...
		<span class="small">
		<?php echo '<strong>Scripture: </strong>'.$scripture1.' - '.$scripture2;?>
		<?php if ($this->params->get('show_teacher_view') > 0){ ?>
			<?php if (!$this->mediaplayer->teachername) {
			}
			else { ?>
				<?php echo '<strong> By: </strong>';?>
				<?php echo $this->mediaplayer->teachername; ?>
			<?php } ?>
		<?php } ?>	
		</span>
		&nbsp;
		...
		<td valign="top" >
			<?php
				...
				switch ($df)
					{
						case 0:
							$date	= date('M j, Y', strtotime($this->mediaplayer->studydate));
						break;
						case 1:
							$date	= date('M j', strtotime($this->mediaplayer->studydate) );
						break;
						case 2:
							$date	= date('n/j/Y',  strtotime($this->mediaplayer->studydate));
						break;
						case 3:
							$date	= date('n/j', strtotime($this->mediaplayer->studydate));
						break;
						case 4:
							$date	= date('l, F j, Y',  strtotime($this->mediaplayer->studydate));
						break;
						case 5:
							$date	= date('F j, Y',  strtotime($this->mediaplayer->studydate));
						break;
						case 6:
							$date = date('j F Y', strtotime($this->mediaplayer->studydate));
						break;
						case 7:
							$date = date('j/n/Y', strtotime($this->mediaplayer->studydate));
						break;
						case 8:
							$date = JHTML::_('date', $this->mediaplayer->studydate, JText::_('DATE_FORMAT_LC'));
						break; 
						default:
							$date = date('n/j', strtotime($this->mediaplayer->studydate));
						break;
			...
			<?php if ($this->params->get('show_series_view') > 0){ ?>
				<?php if (!$this->mediaplayer->stext) { ?>
				<?php }
					else { ?>
					<span class="small">
					<?php echo '<strong>'.JText::_('Series: ').'</strong>'.$this->mediaplayer->stext; ?>
					</span>
				<?php } ?>
				
			<?php } ?>
			<?php if ($this->params->get('show_studynumber_view') > 0){ ?>
				<?php if (!$this->mediaplayer->studynumber) {
				}
				else { ?>
				<span class="small">
				<?php echo '<strong>'.JText::_('Study Number: ').'</strong>'.$this->mediaplayer->studynumber;?>
				</span>
				<?php } ?>
			<?php } ?>
	...
	<?php if ($this->params->get('show_description_view') > 0){ ?>
		<tr>
			<td valign="top">
			<?php if (!$this->mediaplayer->studyintro) {
			}
			else { ?>
				<span class="small">
					<?php echo $this->mediaplayer->studyintro; ?>
				</span>
			<?php } ?>
			</td>
		</tr></table>
...
//		. ' WHERE #__bsms_mediafiles.study_id = '.$this->mediaplayer->mfid;
		. ' WHERE #__bsms_mediafiles.study_id = '.$this->mediaplayer->study_id;
...
<?php endif; //end of if params show media ?>	
		...
		<td>
			<br />
			<?php // Here comes the media player
			echo $this->output;?>
		</td>
		</tr>
	...
	<?php if ($this->params->get('show_text_view') > 0): ?>
		...
		<td>
			<br />
			<?php echo $this->mediaplayer->studytext; ?>
		</td>
		</tr>
	<?php endif; ?>	
...
<input type="hidden" name="study_id" value="<?php echo $this->mediaplayer->sid;?>" />
...
<input type="hidden" name="study_detail_id" value="<?php echo $this->mediaplayer->sid;?>"  />
...
?>
	...
	if ($this->menuid){
	$link = JRoute::_('index.php?option='.$option.'&view=studieslist&Itemid='.$this->menuid);}
	?>
	<a href="<?php echo $link;?>">&lt; <?php echo $link_text; ?></a>
	
<input type="hidden" name="option" value="com_biblestudy" />

<input type="hidden" name="id" value="<?php echo $this->mediaplayer->id; ?>" />
<input type="hidden" name="task" value="" />
<input type="hidden" name="controller" value="mediaplayer" />
</form>
