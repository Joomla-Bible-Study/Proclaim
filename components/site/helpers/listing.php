...
function getListing($row, $params, $oddeven)
{
	...
	$listing .= '">'; 
		...
		//dump ($params->get('row1col1'), 'elementid: ');
		 ...
		 $listing .= getCell($elementid->id, $elementid->element, $rowcolid, $colspan, $rowspan, $lastcol);
...
function getElementid($rowid, $row, $params)
	{
	 ...
	 case 1:
		...
		$esv = 0;
		$elementid->element = getScripture($params, $row, $esv);
		break;
	case 2:
...
function getCell($elementid, $element, $rowcolid, $colspan, $rowspan, $lastcol)
	{
		$cell .= '
