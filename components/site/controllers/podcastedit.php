  ...
  <channel>
    ...
    <ttl>1</ttl>';
		...
		$query = 'SELECT p.id AS pid, p.podcastlimit,'
			...
			. ' WHERE mf.podcast_id = '.$cid.' ORDER BY createdate DESC '.$limit;
			$db->setQuery( $query );
      ...
      <title>'.$title.'</title>
      <link>http://'.$episode->server_path.$episode->folderpath.$episode->filename.'</link>
      ...
      <itunes:explicit>no</itunes:explicit>
		...
		if (!$filecontent) {
			$mainframe->redirect($link, JText::_('Operation Failed').': '.JText::_('Content empty.'));
		}
		...
		if ($return)
		{
			...
			{
				case 'apply_source':
					$mainframe->redirect($link, JText::_($podinfo->filename.' saved'));
					break;
			...
			$mainframe->redirect($link, JText::_('Operation Failed').': '.JText::_('Failed to open file for writing.'));
		}
