...
(function () {
    ...
    const getSeparatorOptions = () => {
        if (window.Joomla && typeof Joomla.getOptions === 'function') {
            const phpOptions = Joomla.getOptions('com_proclaim.separatorOptions');
            if (phpOptions && Array.isArray(phpOptions)) {
                return phpOptions;
            }
        }
    ...
    class LayoutEditor {
        ...
        constructor(container, options = {}) {
            ...
            if (this.options.contexts && this.options.contexts.length === 1) {
                [this.options.context] = this.options.contexts;
        ...
        switchContext(context) {
            ...
            try { sessionStorage.setItem(this._contextStorageKey, context); } catch { /* ignore */ }
        ...
        updateLandingSectionState(sectionId, enabled) {
            ...
            if (!data) {
                data = { enabled, order: this.state.size + 1 };
        ...
        createElementCard(element, isPalette = false) {
            ...
            const deprecatedBadge = element.deprecated ? ' <span class="badge bg-warning text-dark" style="font-size:10px;vertical-align:middle;">deprecated</span>' : '';
        ...
        onElementAdded(evt) {
            ...
            if (evt.item.dataset.paletteItem === 'true' || evt.item.dataset.paletteItem === '') {
                ...
                if (element) {
                    const depBadge = element.deprecated ? ' <span class="badge bg-warning text-dark" style="font-size:10px;vertical-align:middle;">deprecated</span>' : '';
                    evt.item.innerHTML = `
                        <span class="element-handle"><span class="icon-menu" aria-hidden="true"></span></span>
