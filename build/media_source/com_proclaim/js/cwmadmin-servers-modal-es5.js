(function () {
   ...
   */
  (function () {
    window.jSelectServer = function (id, title, object, link, lang) {
      var hreflang = '';
      ...
      for (var i = 0, l = elements.length; l > i; i += 1) {
        elements[i].addEventListener('click', function (event) {
          ...
          if (window.parent.Joomla.Modal) {
            ...
            var theForm = doc.getElementById("adminForm");
            var task = doc.getElementsByName('task');
            for (var j = 0; j < task.length; j++) {
              task[j].value = "cwmmediafile.setServer";
            }
            theForm.submit();
          }
        });
