...
class Pkg_JBSMInstallerScript {
	...
	public function preflight($type, $parent) {
		...
		$logFile = JFactory::getConfig()->get('log_path').'/jbsm.php';
		if (is_file($logFile)) {
			@unlink($logFile);
		}
	...
	public function postflight($type, $parent) {
		...
		if (version_compare(JVERSION, '3.0', '>')) {
			$modal = <<<EOS
<div id="jbsm-modal" class="modal hide fade" style="width:34%;margin-left:-20%;top:25%;"><div class="modal-body"></div></div><script>jQuery('#jbsm-modal').remove().prependTo('body').modal({backdrop: 'static', keyboard: false, remote: '{$this->makeRoute('index.php?option=com_biblestudy&view=install&format=raw')}'})</script>
EOS;

		} else {
			$modal = "<script>window.addEvent('domready',function(){SqueezeBox.open('{$this->makeRoute('index.php?option=com_biblestudy&view=install&tmpl=component')}',{size:{x:530,y:140},sizeLoading:{x:530,y:140},closable:false,handler:'iframe'});});</script>";
		}
		$app->enqueueMessage('Installing BibleStudy... '.$modal);
	...
	protected function checkJBSM($version) {
		...
		$table = $db->getPrefix().'jbsm_version';
		...
		if (class_exists('JBSMInstaller')) {
			if (JBSMInstaller::canDowngrade($version)) return true;
		} else {
	...
	protected function fixUpdateSite() {
		...
		$query = $db->getQuery(true)
			...
			->where($db->quoteName('location') . ' LIKE '. $db->quote('http://www.joomlabiblstudy.org/%'))
			->order($db->quoteName('update_site_id') . ' ASC');
		$db->setQuery($query);
		$list = (array) $db->loadColumn();

		$query = $db->getQuery(true)
			->set($db->quoteName('name').'='.$db->quote('BibleStudy Update Site'))
			->set($db->quoteName('type').'='.$db->quote('collection'))
			->set($db->quoteName('location').'='.$db->quote('http://www.joomlabiblestudy.org/index.php?option=com_ars&view=update&task=stream&format=xml&id=2&dummy=extension.xml'))
		...
		// Currently only pkg_biblstudy gets registered to update site, so remove everything else.
		$list[] = $id;
		$ids = implode(',', $list);
