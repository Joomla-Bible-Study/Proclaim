-- com_proclaim's schema as it stood at 10.0.0.
--
-- The second half of the cwm-schema-replay baseline: the oldest release this
-- project still supports upgrading from, which is what makes replaying all 37
-- migrations meaningful rather than arbitrary.
--
-- Copied verbatim from `git show v10.0.0:admin/sql/install.mysql.utf8.sql`,
-- with only this header added. It is committed rather than extracted at run
-- time on purpose: a baseline pulled out of git history when the check runs is
-- not reviewable in a pull request, and this file decides what "every
-- migration passes" means. Changing it changes the claim.
--
-- Do not "fix" anything in here. Its value is being wrong in exactly the ways
-- a 10.0.0 site is wrong -- that is the schema the migrations have to cope
-- with. Every defect the replay finds is a defect a real upgrading site meets.
--
-- Paired with joomla-core.sql, which supplies the core tables the migrations
-- write to and this file does not create. Order matters: core first.
--
-- Raising the floor (dropping 10.0.x support) means replacing this with the
-- install SQL of the new oldest supported release and moving `from` to match,
