    ...
    function update($parent)
    {
        ...
        foreach ($data as $d)
        {
            ...
            if ($registry->get('useexpert_module') > 0)
            {
                ...
                $filecontent = '<?php defined(\'_JEXEC\') or die; ?>' . $dataheaderlist .'<?php foreach ($list as $study){ ?>'. $dataitemlist.'<?php } ?>';
                $filename = 'default_moduletemplate_'.$filenumber;
                $file = JPATH_ROOT . DIRECTORY_SEPARATOR . 'modules/mod_biblestudy/tmpl' . DIRECTORY_SEPARATOR . $filename.'.php';
