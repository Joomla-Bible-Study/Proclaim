    ...
    private function playlistSyncTask(ExecuteTaskEvent $event): int
    {
        ...
        try {
            ...
            if ($pushChanges) {
                ...
                if ($dryRun) {
                    foreach ($stats['descriptionsWouldPush'] as $would) {
                        $this->logTask('  ~ ' . $would);
                    }
                } elseif ($stats['descriptionsPushed'] > 0) {
                    $this->logTask(Text::sprintf(
                        'PLG_TASK_PROCLAIM_PLAYLISTSYNC_DESC_PUSHED',
                        $stats['descriptionsPushed']
                    ));
                }

                if ($dryRun) {
                    foreach ($stats['membershipsWouldPush'] as $would) {
                        $this->logTask('  ~ ' . $would);
                    }
                } elseif ($stats['membershipsPushed'] > 0) {
                    $this->logTask(Text::sprintf(
                        'PLG_TASK_PROCLAIM_PLAYLISTSYNC_MEMBERS_PUSHED',
                        $stats['membershipsPushed']
                    ));
                }
